<?php
	...
	class GAConfig {
		...
		public static function getConf() {
			if(self::$conf == null) {
				...
				if(defined('OPTION_TYPE_TEXTBOX')) {
					...
					$const['OPTION_TYPE_SELECTOR'] = 5;
					$const['OPTION_TYPE_RADIO'] = 4;
					}
				...
				self::$conf = array (
					...
					'TrackPageViewsPosition' => array(
						...
						'<li>' . gettext_pl('between &lt;head&gt;&lt;/head&gt; tags','google_analytics') . '</li>' .
						'<li>' . gettext_pl('after the &lt;body&gt; tag','google_analytics') . '</li>' .
						'<li>' . gettext_pl('before the &lt;/body&gt; tag','google_analytics') . '</li></ul>',
						),
		...
		public static function buildLinkerParam($domainlist) {
			return "['" . str_replace(",","','",$domainlist) . "']";
		}
	}
?>
